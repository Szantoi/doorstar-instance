...
async function verifyStaticContent() {
  ...
  const loginPanelStart = indexText.indexOf('data-view-panel="login"');
  const dashboardPanelStart = indexText.indexOf('data-view-panel="dashboard"');
  const loginPanelMarkup = indexText.slice(loginPanelStart, dashboardPanelStart);
  assert.ok(loginPanelStart >= 0 && dashboardPanelStart > loginPanelStart);
  assert.doesNotMatch(loginPanelMarkup, /<input\b/i);
  assert.doesNotMatch(indexText, /type="password"/i);
  assert.match(indexText, /data-view-target="dashboard"/);
  assert.match(indexText, /data-view-target="users"/);
  assert.match(indexText, /data-view-panel="users"/);
  assert.match(indexText, /data-view-panel="project"/);
  assert.match(indexText, /id="preview-add-user"/);
  assert.match(indexText, /id="preview-user-dialog"/);
  assert.match(indexText, /id="preview-user-name"[^>]*type="text"/);
  assert.match(indexText, /id="preview-user-email"[^>]*type="email"/);
  assert.match(indexText, /id="preview-user-role"/);
  assert.match(indexText, /id="preview-user-manager"[^>]*type="checkbox"/);
  assert.match(indexText, /id="preview-user-feedback"[^>]*hidden/);
  assert.match(indexText, /Nem jön létre fiók, meghívó, munkamenet vagy hálózati kérés\./);
  assert.match(appText, /const previewData/);
  assert.match(appText, /const projectPreviewPath = "\/office\/projects\/DS-26133"/);
  assert.match(appText, /const roleLabels/);
  assert.match(appText, /function renderUsers\(\)/);
  assert.match(appText, /function isOnlyEffectiveRosterManager\(user\)/);
  assert.match(appText, /A minta védi az utolsó kijelölt kezelőt/);
  assert.match(appText, /example\.invalid/);
  ...
  assert.doesNotMatch(appText, /\b(localStorage|sessionStorage|document\.cookie)\b/);
...
try {
  ...
  console.log("[doorstar-ui-preview] PASS: static preview, local-only user-admin sample, explicit project fixture, loopback-only listener, and no API route verified.");
} catch (error) {
